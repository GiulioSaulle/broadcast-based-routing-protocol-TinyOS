...
#line 741
typedef int (*__compar_fn_t)(const void *arg_0xb7bb6458, const void *arg_0xb7bb65f0);
#line 779
__extension__ 
#line 796
__extension__ 
...
#line 260
extern double erfc(double arg_0xb7b678c8) __attribute((__leaf__)) __attribute((__nothrow__)) ;
#line 336
__extension__ 
...
unsigned int (*hashfunction)(void *arg_0xb7a11840), 
int (*key_eq_fn)(void *arg_0xb7a11c88, void *arg_0xb7a11e00));
#line 103
#line 102
int 
...
static inline void RandomInitialise(int arg_0xb78e09b8, int arg_0xb78e0b10);
static double RandomUniform(void );
...
const uint8_t RadioRouteC$person_code[1000][8];


uint16_t RadioRouteC$person_code_digit[1000];


uint16_t RadioRouteC$recived_msg_counter[1000];
...
typedef struct RadioRouteC$__nesc_unnamed4334 {
  ...
  uint16_t nextHop;
  uint8_t cost;
} RadioRouteC$RoutingTableEntry;


RadioRouteC$RoutingTableEntry RadioRouteC$routingTable[1000][6];

static bool RadioRouteC$isRouteAvailable(uint16_t destination);
static inline bool RadioRouteC$isCostLower(uint16_t destination, uint16_t cost);
static inline void RadioRouteC$updateRoutingTable(uint16_t destination, uint16_t nextHop, uint16_t cost);
static uint16_t RadioRouteC$getNextHop(uint16_t destination);
static inline uint16_t RadioRouteC$getCost(uint16_t destination);
...
{
  LedsP$Led2$toggle();
  sim_log_debug(232U, "LedsC", "LEDS: Led""2"" %s.\n", LedsP$Led2$get() ? "off" : "on");
#line 116
  ;
}
...
{
  LedsP$Led1$toggle();
  sim_log_debug(229U, "LedsC", "LEDS: Led""1"" %s.\n", LedsP$Led1$get() ? "off" : "on");
#line 101
  ;
}
...
{
  LedsP$Led0$toggle();
  sim_log_debug(226U, "LedsC", "LEDS: Led""0"" %s.\n", LedsP$Led0$get() ? "off" : "on");
#line 86
  ;
}
...
#line 548
{
  ...
  for (i = 0; i < 6; i++) {
      if (RadioRouteC$routingTable[sim_node()][i].destination == destination) {
          RadioRouteC$routingTable[sim_node()][i].nextHop = nextHop;
          RadioRouteC$routingTable[sim_node()][i].cost = cost;
          return;
        }
    }

  for (i = 0; i < 6; i++) {
      if (RadioRouteC$routingTable[sim_node()][i].destination == 0) {
          RadioRouteC$routingTable[sim_node()][i].destination = destination;
          RadioRouteC$routingTable[sim_node()][i].nextHop = nextHop;
          RadioRouteC$routingTable[sim_node()][i].cost = cost;
          return;
        }
...
#line 537
  for (i = 0; i < 6; i++) {
      if (RadioRouteC$routingTable[sim_node()][i].destination == destination) {
          if (RadioRouteC$routingTable[sim_node()][i].cost > cost) {
              return TRUE;
            }
        }
    }
...
#line 588
  for (i = 0; i < 6; i++) {
      if (RadioRouteC$routingTable[sim_node()][i].destination == destination) {
          return RadioRouteC$routingTable[sim_node()][i].cost;
        }
    }
...
#line 289
    {
      ...
      sim_log_debug(176U, "radio_rec", "Received packet at time %s\n", sim_time_string());



      type = __nesc_ntoh_uint8(receivedMsg->type.nxdata);
      sender = __nesc_ntoh_uint16(receivedMsg->sender.nxdata);
      destination = __nesc_ntoh_uint16(receivedMsg->destination.nxdata);
      ...
      switch (type) {
          case 0: 
            ...
            sim_log_debug(177U, "radio_rec", "Node %hu received a Data message\n", TOS_NODE_ID);
          sim_log_debug(178U, "radio_pack", ">>>Pack \n");
          sim_log_debug_clear(179U, "radio_pack", "\t\t Payload Received\n");
          sim_log_debug_clear(180U, "radio_pack", "\t\t sender: %hu \n ", __nesc_ntoh_uint16(receivedMsg->sender.nxdata));
          sim_log_debug_clear(181U, "radio_pack", "\t\t destination: %hu \n ", __nesc_ntoh_uint16(receivedMsg->destination.nxdata));
          sim_log_debug_clear(182U, "radio_pack", "\t\t value: %hu \n ", __nesc_ntoh_uint16(receivedMsg->value.nxdata));


          if (RadioRouteC$isRouteAvailable(destination)) {
              if (!RadioRouteC$generate_send(RadioRouteC$getNextHop(destination), bufPtr, __nesc_ntoh_uint8(receivedMsg->type.nxdata))) {

                  sim_log_debug(183U, "radio_send", "Failed to schedule message transmission\n");
                }
              else 
#line 319
                {
                  sim_log_debug(184U, "radio_send", "Scheduled Data message transmission from Node %hu\n", TOS_NODE_ID);
                }
            }
          ...
          case 1: 
            ...
            sim_log_debug(185U, "radio_rec", "Node %hu received a Route Request message\n", TOS_NODE_ID);
          sim_log_debug(186U, "radio_pack", ">>>Pack \n");
          sim_log_debug_clear(187U, "radio_pack", "\t\t Payload Received\n");
          sim_log_debug_clear(188U, "radio_pack", "\t\t node requested: %hu \n ", __nesc_ntoh_uint16(receivedMsg->nodeRequested.nxdata));


          node_requested = __nesc_ntoh_uint16(receivedMsg->nodeRequested.nxdata);


          if (node_requested == TOS_NODE_ID) {


              if (RadioRouteC$route_rep_sent[sim_node()] == TRUE) {
                  sim_log_debug(189U, "radio_rec", "Node %hu already sent a Route Reply message\n", TOS_NODE_ID);
                  break;
                }

              sim_log_debug(190U, "radio_rec", "Node %hu received a Route Request message for itself\n", TOS_NODE_ID);

              __nesc_hton_uint8(receivedMsg->type.nxdata, 2);
              __nesc_hton_uint16(receivedMsg->sender.nxdata, TOS_NODE_ID);
              __nesc_hton_uint8(receivedMsg->cost.nxdata, 1);


              if (!RadioRouteC$generate_send(sender, bufPtr, __nesc_ntoh_uint8(receivedMsg->type.nxdata))) {

                  sim_log_error(191U, "radio_send", "Failed to schedule message transmission\n");
                }
              else 
#line 355
                {
                  sim_log_debug(192U, "radio_send", "Scheduled message Route Reply from Node %hu\n", TOS_NODE_ID);
                }
            }
          else {
#line 358
            if (RadioRouteC$isRouteAvailable(node_requested)) {


                if (RadioRouteC$route_rep_sent[sim_node()] == TRUE) {
                    sim_log_debug(193U, "radio_rec", "Node %hu already sent a Route Reply message\n", TOS_NODE_ID);
                    break;
                  }

                __nesc_hton_uint8(receivedMsg->type.nxdata, 2);
                __nesc_hton_uint16(receivedMsg->sender.nxdata, TOS_NODE_ID);
                __nesc_hton_uint8(receivedMsg->cost.nxdata, RadioRouteC$getCost(node_requested) + 1);


                if (!RadioRouteC$generate_send(sender, bufPtr, __nesc_ntoh_uint8(receivedMsg->type.nxdata))) {

                    sim_log_error(194U, "radio_send", "Failed to schedule message transmission\n");
                  }
                else 
#line 374
                  {
                    sim_log_debug(195U, "radio_send", "Scheduled message Route Reply from Node %hu\n", TOS_NODE_ID);
                  }
              }
            else 
              {
                ...
                if (RadioRouteC$route_req_sent[sim_node()] == TRUE) {
                    sim_log_debug(196U, "radio_rec", "Node %hu already sent a Route Request message\n", TOS_NODE_ID);
                    break;
                  }

                __nesc_hton_uint8(receivedMsg->type.nxdata, 1);
                if (!RadioRouteC$generate_send(AM_BROADCAST_ADDR, bufPtr, __nesc_ntoh_uint8(receivedMsg->type.nxdata))) {

                    sim_log_error(197U, "radio_send", "Failed to schedule message transmission\n");
                  }
                else 
#line 391
                  {
                    sim_log_debug(198U, "radio_send", "Scheduled message Route Request Forward from Node %hu\n", TOS_NODE_ID);
                  }
              }
            }
          break;
          case 2: 
            ...
            sim_log_debug(199U, "radio_rec", "Node %hu received a Route Reply message\n", TOS_NODE_ID);
          sim_log_debug(200U, "radio_pack", ">>>Pack \n");
          sim_log_debug_clear(201U, "radio_pack", "\t\t Payload Received\n");
          sim_log_debug_clear(202U, "radio_pack", "\t\t sender: %hu \n ", __nesc_ntoh_uint16(receivedMsg->sender.nxdata));
          sim_log_debug_clear(203U, "radio_pack", "\t\t node requested: %hu \n ", __nesc_ntoh_uint16(receivedMsg->nodeRequested.nxdata));
          sim_log_debug_clear(204U, "radio_pack", "\t\t cost: %hhu \n ", __nesc_ntoh_uint8(receivedMsg->cost.nxdata));


          node_requested = __nesc_ntoh_uint16(receivedMsg->nodeRequested.nxdata);
          cost = __nesc_ntoh_uint8(receivedMsg->cost.nxdata);



          if (node_requested == TOS_NODE_ID) {
              sim_log_debug(205U, "radio_rec", "Node %hu received a Route Reply message for itself\n", TOS_NODE_ID);
              break;
            }
          else {
#line 415
            if (!RadioRouteC$isRouteAvailable(node_requested) || RadioRouteC$isCostLower(node_requested, cost)) {


                RadioRouteC$updateRoutingTable(node_requested, sender, cost);
                sim_log_debug(206U, "radio_rec", "Node %hu updated the routing table with destination %hu, next hop %hu and cost %hhu\n", TOS_NODE_ID, node_requested, sender, cost);


                if (RadioRouteC$route_rep_sent[sim_node()] == TRUE) {
                    sim_log_debug(207U, "radio_rec", "Node %hu already sent a Route Reply message\n", TOS_NODE_ID);
                    break;
                  }


                __nesc_hton_uint8(receivedMsg->cost.nxdata, cost + 1);
                __nesc_hton_uint16(receivedMsg->sender.nxdata, TOS_NODE_ID);
                if (!RadioRouteC$generate_send(AM_BROADCAST_ADDR, bufPtr, type)) {

                    sim_log_error(208U, "radio_send", "Failed to schedule route_reply transmission\n");
                  }
                else 
#line 433
                  {
                    sim_log_debug(209U, "radio_send", "Scheduled route_reply transmission from Node %hu\n", TOS_NODE_ID);
                  }
              }
            else {
#line 437
              if (TOS_NODE_ID == 1 && !RadioRouteC$data_sent[sim_node()]) {

                  __nesc_hton_uint8(receivedMsg->type.nxdata, 0);
                  __nesc_hton_uint16(receivedMsg->sender.nxdata, TOS_NODE_ID);
                  __nesc_hton_uint16(receivedMsg->destination.nxdata, 7);
                  __nesc_hton_uint16(receivedMsg->value.nxdata, 5);

                  if (!RadioRouteC$generate_send(RadioRouteC$getNextHop(__nesc_ntoh_uint16(receivedMsg->destination.nxdata)), bufPtr, __nesc_ntoh_uint8(receivedMsg->type.nxdata))) {

                      sim_log_error(210U, "radio_send", "Failed to schedule message transmission\n");
                    }
                  else 
#line 447
                    {
                      sim_log_debug(211U, "radio_send", "Scheduled message Data from Node %hu\n", TOS_NODE_ID);
                    }
                }
              }
            }
          ...
          default: 
            ...
            sim_log_error(212U, "radio_rec", "Unknown message type %hhu\n", type);
          break;
        }



      RadioRouteC$person_code_digit[sim_node()] = RadioRouteC$recived_msg_counter[sim_node()] % 8;

      switch (RadioRouteC$person_code[sim_node()][RadioRouteC$person_code_digit[sim_node()]] % 3) {
          case 0: 
            RadioRouteC$Leds$led0Toggle();
          sim_log_debug(213U, "led_0", "Received packet %hu, person code digit %hu, activating LED 0\n", RadioRouteC$recived_msg_counter[sim_node()], RadioRouteC$person_code[sim_node()][RadioRouteC$person_code_digit[sim_node()]]);
          break;
          case 1: 
            RadioRouteC$Leds$led1Toggle();
          sim_log_debug(214U, "led_1", "Received packet %hu, person code digit %hu, activating LED 1\n", RadioRouteC$recived_msg_counter[sim_node()], RadioRouteC$person_code[sim_node()][RadioRouteC$person_code_digit[sim_node()]]);
          break;
          case 2: 
            RadioRouteC$Leds$led2Toggle();
          sim_log_debug(215U, "led_2", "Received packet %hu, person code digit %hu, activating LED 2\n", RadioRouteC$recived_msg_counter[sim_node()], RadioRouteC$person_code[sim_node()][RadioRouteC$person_code_digit[sim_node()]]);
          break;
        }

      sim_log_debug(216U, "led_status", "LED status: %d%d%d\n", RadioRouteC$Leds$get() & LEDS_LED0, (RadioRouteC$Leds$get() & LEDS_LED1) / 2, (RadioRouteC$Leds$get() & LEDS_LED2) / 4);

      sim_log_debug_clear(217U, "radio_rec", "\n-------------------------------------------\n\n");

      RadioRouteC$recived_msg_counter[sim_node()]++;

      return bufPtr;
    }

  sim_log_error(218U, "radio_rec", "Receiving error \n");
...
#line 221
{
  ...
  if (err == SUCCESS) {
      sim_log_debug(168U, "radio", "Radio successfully started\n");


      for (i = 0; i < 6; i++) {
          RadioRouteC$routingTable[sim_node()][i].destination = 0;
          RadioRouteC$routingTable[sim_node()][i].nextHop = 0;
          RadioRouteC$routingTable[sim_node()][i].cost = 255;
        }
...
#line 236
    {
      sim_log_debug(169U, "radio", "Radio failed to start, Trying again\n");
...
#line 242
{
  ...
  sim_log_debug(170U, "radio", "Radio stopped\n");
}
...
# 100 "/opt/tinyos-main-2.1.2/tos/interfaces/Send.nc"
inline static void /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$Send$sendDone(uint8_t arg_0xb7493010, message_t * msg, error_t error){
#line 100
  switch (arg_0xb7493010) {
#line 100
    case 0U:
#line 100
...
#line 100
    default:
#line 100
      /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$Send$default$sendDone(arg_0xb7493010, msg, error);
#line 100
      break;
#line 100
    }
...
# 75 "/opt/tinyos-main-2.1.2/tos/interfaces/TaskBasic.nc"
inline static void SimSchedulerBasicP$TaskBasic$runTask(uint8_t arg_0xb783c650){
#line 75
  switch (arg_0xb783c650) {
#line 75
...
#line 75
    default:
#line 75
      SimSchedulerBasicP$TaskBasic$default$runTask(arg_0xb783c650);
#line 75
      break;
#line 75
    }
...
#line 133
{
  ...
  radio_route_msg_t *payload = (radio_route_msg_t *)RadioRouteC$Packet$getPayload(packet, sizeof(radio_route_msg_t ));

  if (RadioRouteC$locked[sim_node()]) {
      sim_log_error(148U, "radio_send", "Radio is locked\n");
      return FALSE;
    }

  switch (__nesc_ntoh_uint8(payload->type.nxdata)) {


      case 0: 
        sim_log_debug(149U, "radio_pack", ">>>Sending DATA_MESSAGE to next hop\n");
      sim_log_debug_clear(150U, "radio_pack", "\t\t sender: %hu \n ", __nesc_ntoh_uint16(payload->sender.nxdata));
      sim_log_debug_clear(151U, "radio_pack", "\t\t destination: %hu \n ", __nesc_ntoh_uint16(payload->destination.nxdata));

      if (RadioRouteC$AMSend$send(RadioRouteC$getNextHop(__nesc_ntoh_uint16(payload->destination.nxdata)), packet, sizeof(radio_route_msg_t )) == SUCCESS) {
          RadioRouteC$locked[sim_node()] = TRUE;
          sim_log_debug(152U, "radio_send", "DATA_MESSAGE sent to %d\n", RadioRouteC$getNextHop(__nesc_ntoh_uint16(payload->destination.nxdata)));
          return TRUE;
        }
      else {
          sim_log_debug(153U, "radio_send", "DATA_MESSAGE not sent to %d\n", RadioRouteC$getNextHop(__nesc_ntoh_uint16(payload->destination.nxdata)));
          return FALSE;
        }
      break;
      ...
      case 1: 
        sim_log_debug_clear(154U, "radio_pack", "\n-------------------------------------------\n\n");
      sim_log_debug(155U, "radio_pack", ">>>Sending ROUTE_REQ to broadcast \n");
      sim_log_debug_clear(156U, "radio_pack", "\t\t node requested: %hu \n ", __nesc_ntoh_uint16(payload->nodeRequested.nxdata));

      if (RadioRouteC$AMSend$send(AM_BROADCAST_ADDR, packet, sizeof(radio_route_msg_t )) == SUCCESS) {
          RadioRouteC$locked[sim_node()] = TRUE;
          sim_log_debug(157U, "radio_send", "ROUTE_REQ sent to broadcast\n");
          return TRUE;
        }
      else {
          sim_log_error(158U, "radio_send", "ROUTE_REQ not sent to broadcast\n");
          return FALSE;
        }
      ...
      sim_log_debug(160U, "radio_pack", ">>>Sending ROUTE_REP to broadcast \n");
      sim_log_debug_clear(161U, "radio_pack", "\t\t sender: %hu \n ", __nesc_ntoh_uint16(payload->sender.nxdata));
      sim_log_debug_clear(162U, "radio_pack", "\t\t node requested: %hu \n ", __nesc_ntoh_uint16(payload->nodeRequested.nxdata));
      sim_log_debug_clear(163U, "radio_pack", "\t\t cost: %hu \n ", __nesc_ntoh_uint8(payload->cost.nxdata));

      if (RadioRouteC$AMSend$send(AM_BROADCAST_ADDR, packet, sizeof(radio_route_msg_t )) == SUCCESS) {
          RadioRouteC$locked[sim_node()] = TRUE;
          sim_log_debug(164U, "radio_send", "ROUTE_REP sent to broadcast\n");
          return TRUE;
        }
      else {
          sim_log_error(165U, "radio_send", "ROUTE_REP not sent to broadcast\n");
          return FALSE;
        }
      ...
      default: 
        sim_log_error(166U, "radio_send", "Trying to send an invalid message type\n");
      return FALSE;
      break;
    }
}
...
#line 247
{
  ...
  sim_log_debug(171U, "timer", "Timer1 fired\n");
  if (TOS_NODE_ID == 1) {
      radio_route_msg_t *payload = (radio_route_msg_t *)RadioRouteC$Packet$getPayload(&RadioRouteC$packet[sim_node()], sizeof(radio_route_msg_t ));

#line 254
      if (payload == (void *)0) {

          sim_log_error(172U, "radio_pack", "Failed to obtain payload\n");
        }
      else 
#line 257
        {
          __nesc_hton_uint8(payload->type.nxdata, 1);
          __nesc_hton_uint16(payload->nodeRequested.nxdata, 7);


          if (!RadioRouteC$generate_send(__nesc_ntoh_uint16(payload->destination.nxdata), &RadioRouteC$packet[sim_node()], __nesc_ntoh_uint8(payload->type.nxdata))) {

              sim_log_error(173U, "radio_send", "Failed to schedule message transmission\n");
            }
          else 
#line 265
            {
              sim_log_debug(174U, "radio_send", "Scheduled Route request for Node %hu from Node %hu\n", __nesc_ntoh_uint16(payload->nodeRequested.nxdata), TOS_NODE_ID);
            }
        }
    }
}
...
# 83 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
inline static void /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC$0$Timer$fired(uint8_t arg_0xb7315c70){
#line 83
  switch (arg_0xb7315c70) {
#line 83
    case 0U:
#line 83
...
#line 83
    default:
#line 83
      /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC$0$Timer$default$fired(arg_0xb7315c70);
#line 83
      break;
#line 83
    }
...
#line 162
# 80 "/opt/tinyos-main-2.1.2/tos/interfaces/AMSend.nc"
inline static error_t /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$AMSend$send(am_id_t arg_0xb74939f8, am_addr_t addr, message_t * msg, uint8_t len){
#line 80
  unsigned char __nesc_result;
#line 80

#line 80
  __nesc_result = TossimActiveMessageC$AMSend$send(arg_0xb74939f8, addr, msg, len);
#line 80
...
#line 57
  {
    sim_log_debug(223U, "Init", "LEDS: initialized.\n");
...
#line 215
{
  sim_log_debug(167U, "boot", "Application booted.\n");
...
#line 575
  for (i = 0; i < 6; i++) {
      if (RadioRouteC$routingTable[sim_node()][i].destination == destination) {
          return RadioRouteC$routingTable[sim_node()][i].nextHop;
        }
    }
...
#line 494
{
  ...
  radio_route_msg_t *payload = (radio_route_msg_t *)RadioRouteC$Packet$getPayload(bufPtr, sizeof(radio_route_msg_t ));
  ...
  if (error == SUCCESS) {
      ...
      sim_log_debug(219U, "radio_send", "Packet sent...");
      sim_log_debug_clear(220U, "radio_send", " at time %s \n", sim_time_string());


      if (__nesc_ntoh_uint8(payload->type.nxdata) == 0 && TOS_NODE_ID == 1) {
          RadioRouteC$data_sent[sim_node()] = TRUE;
          sim_log_debug(221U, "radio_send", "Node %hu will not send any more data messages\n", TOS_NODE_ID);
        }
    }
  else 
    {
      sim_log_error(222U, "radio_send", "Send done error!\n");
    }
}
...
# 64 "RadioRouteC.nc"
    [8])&RadioRouteC$person_code[__nesc_mote];
    *size = sizeof(*(unsigned char (*)
#line 64
    [8])&RadioRouteC$person_code[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "RadioRouteC$person_code_digit"))
  {
    *addr = (uintptr_t)&RadioRouteC$person_code_digit[__nesc_mote];
    *size = sizeof(RadioRouteC$person_code_digit[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "RadioRouteC$recived_msg_counter"))
  {
    *addr = (uintptr_t)&RadioRouteC$recived_msg_counter[__nesc_mote];
    *size = sizeof(RadioRouteC$recived_msg_counter[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "RadioRouteC$routingTable"))
  {
    *addr = (uintptr_t)&RadioRouteC$routingTable[__nesc_mote];
    *size = sizeof(RadioRouteC$routingTable[__nesc_mote]);
    return 0;
  }
...
static void __nesc_nido_initialise(int __nesc_mote)
{
  ...
  memcpy((void *)&*(unsigned char (*)[8])&RadioRouteC$person_code[__nesc_mote], (void *)&(unsigned char const [8]){ 1, 0, 9, 2, 8, 5, 0, 6 }, sizeof *(unsigned char (*)[8])&RadioRouteC$person_code[__nesc_mote]);
  memset((void *)&RadioRouteC$person_code_digit[__nesc_mote], 0, sizeof RadioRouteC$person_code_digit[__nesc_mote]);
  RadioRouteC$recived_msg_counter[__nesc_mote] = 0;
  memset((void *)&RadioRouteC$routingTable[__nesc_mote], 0, sizeof RadioRouteC$routingTable[__nesc_mote]);
